    ...
    <main>
      <div class="layer" v-if="this.showDetails" @click.self="toggleDetails()">
        <div class="overlay-in">
          ...
          <div class="action">
            <button @click="editDebtor(this.detailsPerson)">Edit</button>
            <button class="delete" @click="deleteDebtor(this.detailsPerson.Id)">Delete</button>
          </div>
        </div>
      </div>
        ...
        <form @submit.prevent="addDebtor()">
          ...
          <div class="flex">
            <div>
              ...
              <input type="text" v-model="debtorForm.firstname" placeholder="Enter firstname" required>
             </div>
            <div>
              <label>Last Name</label>
              <input type="text" v-model="debtorForm.lastname" placeholder="Enter lastname" required>
            </div>
          </div>
          <div class="flex">
            <div>
              <label>Contacts</label>
              <input type="text" pattern="[0-9]+" v-model="debtorForm.contacts" placeholder="Enter phone" required>
            </div>
            <div>
              <label>Email</label>
              <input type="email" v-model="debtorForm.email" placeholder="Enter email">
            </div>
          </div>
          <label>ID Number</label>
          <input type="number" class="hide-controllers" min="0" pattern=".{11,}" v-model="debtorForm.idnumber" placeholder="Enter ID Number">
          <label>Amount ( RWF )</label>
          <input type="number" min="0" v-model="debtorForm.amount" placeholder="Enter Amount" required>
          <div class="flex">
            <div>
              <label>Made At</label>
              <input type="date" v-model="debtorForm.madeAt" required>
            </div>
            <div>
              <label>Return</label>
              <input type="date" v-model="debtorForm.returnDate" required>
            </div>
          </div>
          <label>Reason</label>
          <input type="text" v-model="debtorForm.reason" placeholder="Enter Reason" id="">
          <button :class="{loading: loadingBtn}">
            <span v-if="loadingBtn">Loading...</span>
            <span v-else>Add Debtor</span>
          </button>
        </form>
      </div>
      ...
      <div class="overlay" v-if="viewedit" @click.self="close()">
        <form @submit.prevent="edittDebtor()">
          <div class="top">
            <h4>Edit Debtor</h4>
            <div class="close" @click="close()">
              <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 50 50"><path d="M9.15625 6.3125L6.3125 9.15625L22.15625 25L6.21875 40.96875L9.03125 43.78125L25 27.84375L40.9375 43.78125L43.78125 40.9375L27.84375 25L43.6875 9.15625L40.84375 6.3125L25 22.15625Z"/></svg>
          ...
          <label>Amount ( RWF )</label>
          <input type="number" min="0" v-model="editForm.Amount" placeholder="Enter Amount" required>
          <div class="flex">
            <div>
              <label>Made At</label>
          ...
          <button :class="{loading: loadingBtn}">
            <span v-if="loadingBtn">Loading...</span>
            <span v-else>Update Debtor</span>
          </button>
        </form>
      </div>
      <div class="overlay" v-if="viewdelete" @click.self="close()">
        <div class="confirm">
          <label>Are you sure you want to delete this Debtor ?</label>
          <div class="bts">
      ...
      <div class="spacedTitle">
        ...
        <button @click.self="viewadd = true">Add Debtor</button>
      </div>
      <p class="center" v-if="loading">Loading...</p>
      <div v-else>
      
        <div class="data-table" v-if="debtors.length > 0">
          <div class="filtering">
            <div class="search">
          ...
          <label v-if="this.paid">All Paid Debtors</label>
          <label v-if="this.notpaid">All Pending Debtors</label>
          <label v-if="searched.length > 0">All Active Debtors "{{ searched }}"</label>
          <label v-else-if="dateFilter">All Active Debtors From "{{ startDate }}" To "{{ endDate }}"</label>
          <label v-else>All Active Debtors</label>
          <div class="table" v-if="this.allDebtorsComp.length > 0">
            <div class="headings">
              <h2>Names</h2>
              <h2>Contacts</h2>
            ...
            <div v-if="this.notpaid">
              <div class="row" v-for="debtor in compPendingDebtors" :key="debtor.id">
                <h3>{{ debtor.Firstname }} {{ debtor.Lastname }}</h3>
                <h3>{{ debtor.Phone }}</h3>
                <h3 class="toHide" v-if="debtor.Email">{{ debtor.Email }}</h3>
                <h3 class="toHide" v-else>N/A</h3>
                <h3 class="toHide">{{ debtor.Amount }} RWF</h3>
                <h3 class="toHide">{{ debtor.MadeDate }}</h3>
                <h3 class="toHide">{{ debtor.ReturnDate }}</h3>
                <div class="toHide">
                  <button class="edit" @click="editDebtor(debtor)">Edit</button>
                  <button class="delete" @click="deleteDebtor(debtor.Id)">Delete</button>
                  <button >
                    Remind
                    <!-- <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="144" height="144"><path d="M10 8C6.86 8 4.2795313 10.42 4.0195312 13.5L24 24.289062L43.980469 13.5C43.720469 10.42 41.14 8 38 8L10 8 z M 4 16.890625L4 34C4 37.31 6.69 40 10 40L38 40C41.31 40 44 37.31 44 34L44 16.890625L24.710938 27.320312C24.490938 27.440312 24.25 27.5 24 27.5C23.75 27.5 23.509062 27.440313 23.289062 27.320312L4 16.890625 z" /></svg> -->
                  </button>
                </div>
                <div class="toShow">
                  <div class="actionBtn" @click="toggleDetails(debtor)">
            ...
            <div v-if="this.paid">
              <div class="row" v-for="debtor in compCompletedDebtors" :key="debtor.id">
                <h3>{{ debtor.Firstname }} {{ debtor.Lastname }}</h3>
                <h3>{{ debtor.Phone }}</h3>
                <h3 class="toHide" v-if="debtor.Email">{{ debtor.Email }}</h3>
                <h3 class="toHide" v-else>N/A</h3>
                <h3 class="toHide">{{ comma(debtor.Amount) }} RWF</h3>
                <h3 class="toHide">{{ debtor.MadeDate }}</h3>
                <h3 class="toHide">{{ debtor.ReturnDate }}</h3>
                <div class="toHide">
                  <button class="edit" @click="editDebtor(debtor)">Edit</button>
            ...
            <div>
              <div class="row" v-for="debtor in allDebtorsComp" :key="debtor.id">
                <h3>{{ debtor.Firstname }} {{ debtor.Lastname }}</h3>
                <h3>{{ debtor.Phone }}</h3>
                <!-- <h3 class="toHide" v-if="debtor.Email">{{ debtor.Email }}</h3>
                <h3 class="toHide" v-else>N/A</h3> -->
                <h3 class="toHide">{{ debtor.number_format }} RWF</h3>
                <h3 class="toHide">{{ debtor.MadeDate }}</h3>
                <h3 class="toHide">{{ debtor.ReturnDate }}</h3>
                <div class="toHide">
                  <button class="details" @click="showMore(debtor)">Details</button>
                  <button class="edit" @click="editDebtor(debtor)">Edit</button>
                  <button class="delete" @click="deleteDebtor(debtor.Id)">Delete</button>
                  <button >
                    Remind
          ...
          <p class="center" v-else>No Debtors To Show</p>
        </div>
        <p class="center" v-else>No Active Debtors</p>
      </div>
      <div>
        <vue3-html2pdf
          :show-layout="false"
        ...
        >
          <template  v-slot:pdf-content>
            <div style="padding: 15px 10px;">
              ...
              <p v-if="searched.length > 0" style="font-size: 16px; text-align: center; margin-bottom: 10px; text-decoration: underline;">All Active Debtors "{{ searched }}"</p>
              <p v-else-if="dateFilter" style="font-size: 16px; text-align: center; margin-bottom: 10px; text-decoration: underline;">All Active Debtors From "{{ startDate }}" To "{{ endDate }}"</p>
              <p v-else style="font-size: 16px; text-align: center; margin-bottom: 10px; text-decoration: underline;">Report Of All Active Debtors</p>
              <table ref="pdfContent" style="width: 100%;" cellspacing="0">
                ...
                <tr v-for="(debtor, index) in allDebtorsComp" :key="index">
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'" v-if="debtor.Id_Number != ''">{{ debtor.Id_Number }}</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'" v-else>N/A</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'">{{ debtor.Firstname }} {{ debtor.Lastname }}</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'">{{ debtor.Phone }}</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'">{{ debtor.number_format }} RWF</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'">{{ debtor.MadeDate }}</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px;text-transform: capitalize;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'">{{ debtor.ReturnDate }}</td>
                  <!-- <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px; width: 30%;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'" v-if="debtor.Reason != ''">{{ debtor.Reason }}</td>
                  <td style="font-size: 14.5px; text-align: center; color: #252525; padding: 7px 2px; width: 30%;" :style="index == allDebtorsComp.length - 1 ? '' : 'border-bottom: 1px solid rgba(128, 128, 128, 0.253);'" v-else>N/A</td> -->
                </tr>
              </table>
      ...
      <button @click="generateReport" class="printPdf" :class="{loading: this.loadingBtn}" v-if="allDebtorsComp.length > 0">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      close(){
        this.debtorForm.contacts = '',
        this.debtorForm.firstname = '',
        this.debtorForm.lastname = '',
        this.debtorForm.email = '',
        this.debtorForm.amount = '',
        this.debtorForm.madeAt = '',
        this.debtorForm.returnDate = ''
        this.debtorForm.idnumber = ''
        this.debtorForm.reason = ''
      ...
      addDebtor() {
        this.loadingBtn = true
        this.debtorForm.userid = this.$logged().UserId
        this.$store.dispatch("addDebtor", this.debtorForm)
          .then(resp => {
            if(resp.data.status == 'ok'){
              this.$notify({
      ...
      confirmDelete() {
        this.loadingBtn = true
        this.$store.dispatch("deleteDebtor", {debtorid: this.deleteid})
          .then(resp => {
            if(resp.data.status == 'ok'){
              this.$notify({
                title: resp.data.message,
      ...
      edittDebtor() {
        this.loadingBtn = true
        this.$store.dispatch("updateDebtor", this.editForm)
          .then(resp => {
            if(resp.data.status == 'ok'){
              this.$notify({
                title: resp.data.message,
    ...
    computed: {
      compCompletedDebtors() {
        return this.debtors.filter(each => each.Status == 1);
      },
      compPendingDebtors() {
        return this.debtors.filter(each => each.Status == 0);
      },
      allDebtorsComp() {
        if(this.dateFilter == true) {
          return this.debtors.filter(each => {
          ...
          return this.debtors.filter(each => each.Firstname.toLowerCase().includes(this.searched.toLowerCase()) || each.Lastname.toLowerCase().includes(this.searched.toLowerCase()) || each.Phone.toLowerCase().includes(this.searched.toLowerCase()))
        }
      },
      userLogged() {
        return this.$logged()
